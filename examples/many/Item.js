...
function intent (domSource) {
  return xs.merge(
    ...
    domSource.select('.width-slider').events('input').map(ev => ({
      type: 'CHANGE_WIDTH',
      payload: parseInt(ev.target.value)
    })),
    domSource.select('.remove-btn').events('click').mapTo({ type: 'REMOVE' })
  )
}

function model (props$, action$) {
  const usePropsReducer$ = props$.map(
    props =>
      function usePropsReducer (oldState) {
        return props
      }
  )
  ...
  const changeWidthReducer$ = action$
    ...
    .map(
      action =>
        function changeWidthReducer (oldState) {
  ...
  const changeColorReducer$ = action$
    ...
    .map(
      action =>
        function changeColorReducer (oldState) {
          return { color: action.payload, width: oldState.width }
        }
    )

  return xs
    .merge(usePropsReducer$, changeWidthReducer$, changeColorReducer$)
    .fold((state, reducer) => reducer(state), { color: '#888', width: 200 })
}

function view (state$) {
  return state$.map(({ color, width = 100 }) => {
    const style = {
    ...
    return div('.item', { style }, [
      ...
      div('.slider-container', [
        input('.width-slider', {
          attrs: { type: 'range', min: '100', max: '500', value: width }
        })
      ]),
